...
<kanban:SfKanban AutoGenerateColumns="False"
                 ItemsSource="{Binding TaskDetails}">
    <kanban:KanbanColumn HeaderText="To Do" Categories="Open,New" />
    <kanban:KanbanColumn HeaderText="In Progress" Categories="In Progress" />
    <kanban:KanbanColumn HeaderText="Done" Categories="Closed,Done" />
</kanban:SfKanban>
```
...
kanban.Columns.Add(new KanbanColumn() { HeaderText = "To Do", Categories = "Open,New" });
kanban.Columns.Add(new KanbanColumn() { HeaderText = "In Progress", Categories = "In Progress" });
kanban.Columns.Add(new KanbanColumn() { HeaderText = "Done", Categories = "Closed,Done" });
```
...
2. **MinimumColumnWidth / MaximumColumnWidth** - Bounded auto-sizing
...
<kanban:KanbanColumn HeaderText="⚙️ In Progress" Categories="In Progress" />
...
- `CardsCount` - Number of cards in column
- `AllowedTransitionCategory` - Column's Categories value allowed for transitions
...
**3. Read-Only Archive:**
```xml
<kanban:KanbanColumn HeaderText="Archive" 
                     Categories="Archived" 
                     AllowDrag="False" />
```
...
<Grid>
    <Grid.Resources>
        ...
        <SolidColorBrush x:Key="SyncfusionKanbanPlaceholderBackground" 
                        Color="LightBlue" />
        <SolidColorBrush x:Key="SyncfusionKanbanPlaceholderBorderBrush" 
                        Color="DarkBlue" />
        <Thickness x:Key="SyncfusionKanbanPlaceholderBorderThickness">2</Thickness>
        
        <!-- Drop Placeholder (valid drop location) -->
        <SolidColorBrush x:Key="SyncfusionKanbanPlaceholderSelectedBackground" 
                        Color="LightGreen" />
        <SolidColorBrush x:Key="SyncfusionKanbanPlaceholderSelectedBorderBrush" 
                        Color="DarkGreen" />
...
{ 
    ...
    Categories = "In Progress",
    MinimumCount = 1,    // Must have at least 1 card
    MaximumCount = 5     // Cannot exceed 5 cards
};
```
...
**Error Bar Colors:**
- **Red:** Card count exceeds MaximumCount
- **Yellow/Orange:** Card count below MinimumCount
...
1. **Prevent Overload:** MaximumCount stops too many tasks in one stage
2. **Maintain Flow:** MinimumCount ensures stages aren't empty
3. **Visual Feedback:** Immediate indication of workflow issues
4. **Team Coordination:** Shared understanding of capacity limits

### Best Practices

- Set MaximumCount on bottleneck columns
- Use MinimumCount to ensure continuous flow
...
**Solutions:**
1. Verify MinimumCount/MaximumCount are set on columns
...
**Solutions:**
1. Set MinimumColumnWidth to prevent excessive narrowing
2. Set MaximumColumnWidth to prevent excessive widening
...
2. Check bindings in template (HeaderText, CardsCount)
...
5. **Responsive Widths:** Set MinimumColumnWidth for mobile/tablet support
...
<kanban:SfKanban AutoGenerateColumns="False">
    <kanban:KanbanColumn HeaderText="To Do" Categories="Open" />
    <kanban:KanbanColumn HeaderText="Doing" Categories="In Progress" MaximumCount="3" />
    <kanban:KanbanColumn HeaderText="Done" Categories="Done" AllowDrag="False" />
</kanban:SfKanban>
```
...
<kanban:SfKanban AutoGenerateColumns="False">
    ...
    <kanban:KanbanColumn HeaderText="Development" Categories="In Progress" MaximumCount="5" />
    <kanban:KanbanColumn HeaderText="Code Review" Categories="Review" MaximumCount="3" />
    <kanban:KanbanColumn HeaderText="Testing" Categories="Testing" MaximumCount="4" />
    <kanban:KanbanColumn HeaderText="Done" Categories="Closed,Done" AllowDrag="False" />
</kanban:SfKanban>
```
...
```xml
<kanban:SfKanban AutoGenerateColumns="False">
    <kanban:KanbanColumn HeaderText="New Tickets" Categories="New" />
    <kanban:KanbanColumn HeaderText="In Progress" Categories="Assigned,Working" MaximumCount="10" />
    <kanban:KanbanColumn HeaderText="Waiting" Categories="Pending,Blocked" />
    <kanban:KanbanColumn HeaderText="Resolved" Categories="Resolved,Closed" AllowDrag="False" />
</kanban:SfKanban>
```