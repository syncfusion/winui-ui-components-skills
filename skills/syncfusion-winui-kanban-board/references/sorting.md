...
**Important:** Call `RefreshKanbanColumn()` in CardDrop event to apply sorting after drag-drop:
...
private void OnCardDrop(object sender, KanbanCardDropEventArgs e)
{
    ...
    // Update indexes for all cards in column
    ApplyIndexBasedSorting(e);
    
    // Refresh column to show new order
    kanban.RefreshKanbanColumn(e.TargetColumn.AllowedTransitionCategory.ToString());
}

private void ApplyIndexBasedSorting(KanbanCardDropEventArgs e)
{
...
kanban.SortingOrder = KanbanSortingOrder.Ascending;
// Implement index update logic in CardDrop event
```

## Best Practices

1. **Always Refresh After Drop:** Call RefreshKanbanColumn() in CardDrop event
