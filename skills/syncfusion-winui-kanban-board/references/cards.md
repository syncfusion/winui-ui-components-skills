...
| `IndicatorColorKey` | object | Priority/status key | Color bar on card |
| `Tags` | List<string> | Labels/categories | Bottom of card |
| `Image` | Image | Avatar/icon | Right side of card |
...
var task = new KanbanModel()
{
    ...
    Tags = new List<string>() { "Bug Fixing", "Critical" },
    Image = new Image
    {
...
**Type:** Image control with Source set  
...
<kanban:SfKanban x:Name="kanban"
    ...
    <kanban:SfKanban.IndicatorColorPalette>
        ...
        <kanban:KanbanColorMapping Key="High" Color="Orange"/>
        <kanban:KanbanColorMapping Key="Critical" Color="Red"/>
    </kanban:SfKanban.IndicatorColorPalette>
</kanban:SfKanban>
```
...
indicatorColorPalette.Add(new KanbanColorMapping() { Key="High", Color = Colors.Orange });
indicatorColorPalette.Add(new KanbanColorMapping() { Key="Critical", Color = Colors.Red });
...
var task = new KanbanModel()
{
    IndicatorColorKey = "High",  // Matches "High" key in palette
    // ...other properties
};
```
...
public class KanbanCardTemplateSelector : DataTemplateSelector
{
    ...
    protected override DataTemplate SelectTemplateCore(object item, DependencyObject container)
    {
        ...
        if (task != null)
        {
            if (task.IndicatorColorKey?.ToString() == "High")
                return HighPriorityTemplate;
            else if (task.IndicatorColorKey?.ToString() == "Low")
...
**Solutions:**
1. Check image URI is correct (ms-appx:/// for embedded resources)
...
4. Check image path case (WinUI is case-sensitive)
...
**Solutions:**
1. Verify Tags property is List<string> type
